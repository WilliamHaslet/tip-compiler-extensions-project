...
static cl::opt<bool> dfunc("df", cl::desc("disable function passes"), cl::cat(TIPcat));
static cl::opt<bool> dmod("dm", cl::desc("disable module passes"), cl::cat(TIPcat));
static cl::opt<std::string> disopt("do", cl::desc("disable bitcode optimization"), cl::cat(TIPcat));
static cl::opt<int> optr("or", cl::desc("optimization rounds"), cl::cat(TIPcat));
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    try {
      ...
      if (disopt!="all"){
        Optimizer::optimize(llvmModule.get(), disopt, optr, dfunc, dmod);
      }
