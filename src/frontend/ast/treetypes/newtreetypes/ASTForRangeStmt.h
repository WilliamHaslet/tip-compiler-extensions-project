...
class ASTForRangeStmt : public ASTStmt {
  std::unique_ptr<ASTExpr> ONE, TWO, THREE, FOUR;
  std::unique_ptr<ASTStmt> BODY;
public:
  ASTForRangeStmt(std::unique_ptr<ASTExpr> ONE, std::unique_ptr<ASTExpr> TWO, std::unique_ptr<ASTExpr> THREE, 
      std::unique_ptr<ASTExpr> FOUR, std::unique_ptr<ASTStmt> BODY) : ONE(std::move(ONE)), TWO(std::move(TWO)), 
